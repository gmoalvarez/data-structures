...
//System.out.println(fullName[i]);    
        ...
        list = new Stack<Person>();
        runTests();
    }

    private void runTests() {
        for(int i=1; i <= 5; i++)
            list.push(fullName[i]);
        for(Person x : list)
            System.out.print(x + " \n");
        System.out.println("\n");
        list.makeEmpty();
        try {
            for(int i=0; i < 100; i++)
                list.push(fullName[i]);
            for(int i=99; i >= 0; i--)
                if(((Comparable<Person>)fullName[i]).compareTo(list.pop()) != 0)
        ...
        try {
            for(int i=0; i < 100; i++)
                list.push(fullName[i]);
            for(int i=100; i > 0; i--)
                if(((Comparable<Person>)fullName[i-1]).compareTo(list.pop()) != 0)
                    System.out.println("ERROR, elements don't match");
        }
        catch(Exception e) {
            e.printStackTrace();
...
//===========
        ...
        for(int i=1; i < 50; i++)
            list2.push(new Integer(i));

        // this will remove the top half of the stack. Should only print 1-25 below
        for(int i=2; i <= 25; i++)
            list2.pop();
